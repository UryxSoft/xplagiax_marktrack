...
> ✅ **RESPUESTA**: Sí. El `SemanticAnalyzer` del módulo usará `openrecall/nlp.py` (SentenceTransformer `all-MiniLM-L6-v2`) para generar embeddings de los eventos de texto del estudiante y detectar patrones anómalos.

---

## Open Questions

1. **¿Escala esperada?** → ✅ **250 estudiantes concurrentes**. Buffer de 30 eventos, flush cada 20s, thread pool de 2 workers.

2. **¿Dashboard del profesor?** → ✅ **Solo backend + API por ahora**. El timeline de actividad se implementará en workspace.html en una iteración futura.

3. **¿Feature flag?** → ✅ **`ENABLED = False` por defecto** (deshabilitado para pruebas). Se activa via variable de entorno `OPENRECALL_ENABLED=true`.
