/**
 * review_metrics.js
 * Handles data visualization for the Document Review interface,
 * including the writing timeline and activity charts (Chart.js).
 */
...
(function () {
    ...
    waitForConfig(function (config) {
        const { rawLogs, activityByMinute } = config.metrics;
        ...
        function renderTimelineEvents(logs) {
            ...
            logs.forEach(lg => {
                ...
                const timeStr = (dateObj && !isNaN(dateObj)) ? dateObj.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : (lg.ms ? `T+${Math.round(lg.ms / 1000)}s` : 'Event');
            ...
            try {
                ...
                if (keys.length > 0) {
                    ...
                    function getActivityVal(map, minute) {
                        return (map[minute] !== undefined) ? Number(map[minute]) : ((map[String(minute)] !== undefined) ? Number(map[String(minute)]) : 0);
                    }
        ...
        // Initialize both
        renderTimelineEvents(rawLogs);
        renderReviewMetricsChart(activityByMinute);
